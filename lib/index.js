...
/**
 * Compact a JSON-ish payload into a bounded, readable digest. This server puts
 * its real data in structuredContent.result and only "tool: ok" in the text
 * blocks, so the digest is what makes results actually usable to the model.
 */
export function summarizeJson(value, budget = 1400) {
  let out = ''
  const done = () => out
  const truncated = () => (out.length > budget ? `${out.slice(0, budget)}…` : done())
  ...
  const walk = (v, depth) => {
    ...
    if (typeof v !== 'object') {
      if (typeof v === 'string' && v.length > (depth === 0 ? 400 : 120)) {
        out += JSON.stringify(`${v.slice(0, (depth === 0 ? 400 : 120))}…`)
      } else {
    ...
    if (Array.isArray(v)) {
      ...
      if (v.length > 3) out += `, … ${v.length - 3} more`
      out += ']'
      return
    }
    ...
    if (keys.length > shown) out += `, … ${keys.length - shown} more keys`
    out += '}'
  }
...
export async function callServerTool(client, rawName, args, opts) {
  ...
  if (structured) {
    if (structured.ok === false || result.isError === true) {
      ...
      rendered = [text, code ? `error_code: ${code}` : '', detail ? `error_detail: ${detail}` : '']
        .filter(Boolean)
        .join('\n')
    } else if (structured.result !== undefined && structured.result !== null) {
      const digest = summarizeJson(structured.result)
      if (digest && digest !== '{}') rendered = [text, `result: ${digest}`].filter(Boolean).join('\n')
    }
  }
  return {
    text: rendered,
  ...
  const boot = async () => {
    ...
    const startServer = () => {
      ...
      client.onExit = () => {
        ...
        ctx.logger?.warn?.('[harmonyos-dev-mcp] server exited, unregistering tools and supervising restart')
        // Unregister the previous generation BEFORE it can be replaced, so a
        // fresh registration never collides with stale same-name tools (which
        // previously wiped the whole tool set and caused "unknown tool").
        disposeTools()
        layout.client = null
        scheduleRestart()
      }
    ...
    const layoutConnect = async () => {
      ...
      disposeTools() // defensive: no stale generation may share the new names
      const newDisposers = registerTools(ctx, cfg, tools, () => layout.client)
      layout.disposers = newDisposers
      layout.restartAttempts = 0
      if (newDisposers.length !== tools.length) {
        ctx.logger?.error?.(
          '[harmonyos-dev-mcp] only %d of %d tools registered; check registration errors above',
          newDisposers.length,
          tools.length,
        )
      }
      ctx.logger?.info?.('[harmonyos-dev-mcp] %d tools exposed as %s*', newDisposers.length, publicName(cfg.serverName, ''))
    }
